...
至于黄金的行情数据，我目前无法找到有关的数据，我原打算到上海黄金交易所去爬数据，但是AI说借助akshare包可以直接找到爬取的数据。那么接下来就是安装akshare了。akshare至少需要安装在3.8版本以上的python中，因此我直接在自己电脑的python上安装了akshare。

由于数据访问权限的问题，我现在需要修改我的策略：我需要写出将xlsx中数据导入数据库的脚本，并从akshare中获取黄金价格的数据。接下来的工作围绕新的思路展开，本日志到此结束。
